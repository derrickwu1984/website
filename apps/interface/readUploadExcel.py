...
file_path=settings.UPLOAD_DIR+settings.UPLOAD_FILE_NAME
def check_record():
    ...
    for i in range(len(workbook.sheets())):
        ...
        for row in range(sheet.nrows)[start_index:]:
            sheet_row_values = sheet.row_values(row)
            if (sheet_row_values[0] != '输入' and sheet_row_values[0] != '输出' and row > int(
                    input_start_index) and row < int(input_end_index)):
